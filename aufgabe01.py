...
nan_counts = df_selected.isna().sum().sum() # gesamte NaNs in ausgewählten Spalten
print("Gesamte NaNs in ausgewählten Spalten:", nan_counts)

corr = df_selected.corr(numeric_only=True) # gesamte Korrelationsmatrix
...
X_train_scaled = scaler.fit_transform(X_train)  # Skalieren der Trainingsdaten
...
model.summary() # Modell Zusammenfassung ausgeben

# Trainieren
history = model.fit(
    X_train_scaled, y_train,    # Trainingsdaten
    validation_split=0.1,   # Validierungsdaten
    epochs=20,  # Anzahl der Epochen
    batch_size=128, # Batch-Größe
    verbose=1   # Ausgabe während des Trainings
)
...
for cfg in configs:
    print("\nTraining mit:", cfg)   # cfg ist ein dict mit epochs und batch-size
...
best_cfg, best_val_loss, best_history, best_model = min(results, key=lambda x: x[1])    # best model anhand val_loss auswählen
...
y_predict = best_model.predict(X_test_scaled).flatten()     # Vorhersagen für Testdaten
